//нужна функция, которая подключает стили ошибки
  ...
  const hideItemError = (formElement, inputElement) => {
    const errorElement = formElement.querySelector(`.${inputElement.id}-error`);
      inputElement.classList.remove('form__item_type_error');        // есть баг. если очистить поле и закрыть попап, то при открытии будет готерь ошибка
  ...
  const checkInputValidity = (formElement, inputElement) => {
    if (!inputElement.validity.valid) {
      // Передадим сообщение об ошибке ТРЕТИМ аргументом
  ...
  toggleFormBatton(inputList, saveButton);  //хы. срабатывает один раз.  а надо чтобы при каждом открытии кнопка была неактивна
